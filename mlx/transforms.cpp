...
// C8 (tesseract): flat open-addressing id→degree map for the eval walk.
...
class EvalDegreeMap {
 ...
 private:
  ...
  void rehash(size_t new_capacity) {
    ...
    log2_capacity_ = 0;
    while ((size_t{1} << log2_capacity_) < new_capacity) {
      ++log2_capacity_;
    }
    tombstones_ = 0;
    for (auto& slot : old) {
...
array eval_impl(std::vector<array> outputs, bool async) {
  ...
  while (!tape.empty()) {
    ...
    // needs_fence is empty in the common single-stream case — skip the
    // per-input hash lookups entirely then.
    const bool check_fences = !needs_fence.empty();
    for (auto& in : arr.inputs()) {
      auto nf_it =
          check_fences ? needs_fence.find(in.id()) : needs_fence.end();
