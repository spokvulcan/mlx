...
// Per-call-site memo for the (kernel_name, kernel_source) pairs generated by
// metal_kernel's closure — see the call body for the cache key.
using CustomKernelSignatureCache =
    std::unordered_map<std::string, std::pair<std::string, std::string>>;

// Keep in sync with write_signature's constant/device address-space branch.
...
CustomKernelFunction metal_kernel(
  ...
  return [=,
          ...
          sig_cache_mu = std::make_shared<std::mutex>()](
             const std::vector<array>& inputs,
    ...
    // site they depend only on template_args and the inputs' dtype / ndim /
    // size class (write_signature's size() < max_constant_array_size
    // branch), all of which are constant across calls in practice.
    // Regenerating them every call pays a std::regex construction + replace
    // plus a full signature string build per invocation.
    std::string sig_key;
    for (const auto& [arg_name, arg] : template_args) {
      sig_key += arg_name;
      sig_key += '=';
      if (std::holds_alternative<int>(arg)) {
        sig_key += 'i';
        sig_key += std::to_string(std::get<int>(arg));
      } else if (std::holds_alternative<bool>(arg)) {
        sig_key += 'b';
        sig_key += std::get<bool>(arg) ? '1' : '0';
      } else {
        sig_key += 't';
        sig_key +=
            std::to_string(static_cast<uint32_t>(std::get<Dtype>(arg).val()));
      }
      sig_key += ';';
    }
    sig_key += '|';
    for (const auto& in : inputs) {
      sig_key += std::to_string(static_cast<uint32_t>(in.dtype().val()));
      sig_key += ':';
      sig_key += std::to_string(in.ndim());
      sig_key += ':';
      sig_key += in.size() < max_constant_array_size ? 's' : 'l';
      sig_key += ';';
    }
    sig_key += '|';
    for (const auto& dt : output_dtypes) {
      sig_key += std::to_string(static_cast<uint32_t>(dt.val()));
      sig_key += ';';
    }
    ...
    std::string kernel_source;
    {
      std::lock_guard<std::mutex> lock(*sig_cache_mu);
      if (auto it = sig_cache->find(sig_key); it != sig_cache->end()) {
        kernel_name = it->second.first;
        kernel_source = it->second.second;
      }
    }
    if (kernel_source.empty()) {
      ...
      std::lock_guard<std::mutex> lock(*sig_cache_mu);
      sig_cache->emplace(
          std::move(sig_key), std::make_pair(kernel_name, kernel_source));
    }
